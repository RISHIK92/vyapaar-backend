...
export default async function getFilteredHeroBanners(
  userPincode,
  maxResults = 10
) {
  const allBanners = await getAllEligibleHeroBanners();
...
async function getAllEligibleHeroBanners() {
  const currentDate = new Date();

  return await prisma.adminBanner.findMany({
    where: {
      active: true,
      OR: [
...
async function getBannerLocationData(banner) {
  try {
    ...
    // Check cache first
    const cacheKey = `hero_banner_location_${banner.id}`;
...
export function formatHeroBanners(banners) {
