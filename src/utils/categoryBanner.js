...
export default async function getFilteredCategoryBanners(
  categoryName,
  userPincode,
  maxResults = 10
) {
  ...
  const allBanners = await getAllEligibleCategoryBanners(category.id);

  if (!userPincode) {
    console.log(
      "No pincode provided - returning random category banners from all available"
    );
  ...
  if (!userLocation) {
    console.log(
      `Could not determine location for pincode: ${userPincode} - returning all category banners`
    );
    return shuffleArray(allBanners).slice(0, maxResults);
  }
  ...
  for (const banner of allBanners) {
    if (banner.pincode === 0 || banner.pincode === "000000") {
      ...
      console.log(
        `Global category banner ${banner.id} added (pincode: 000000)`
      );
      continue;
    }
    ...
    if (!bannerLocation) {
      if (!banner.locationUrl && !banner.pincode) {
        ...
        console.log(
          `Unrestricted category banner ${banner.id} added to fallback (no location data)`
        );
      } else {
        console.log(
          `Category banner ${banner.id} skipped (has location fields but couldn't resolve location)`
        );
      }
      continue;
    }
    ...
    if (isInSameArea) {
      ...
      console.log(
        `✓ Area category banner ${banner.id}: ${
      ...
      if (
        ...
        if (distance <= 55) {
          ...
          console.log(
            `→ Nearby category banner ${banner.id}: ${distance.toFixed(
              1
            )}km away (same state)`
          );
        } else {
        ...
        if (!banner.locationUrl && !banner.pincode) {
          ...
          console.log(
            `Category banner ${banner.id} excluded - different state or no coordinates`
          );
        }
      }
    }
  ...
  console.log(
    `Category banner categorization - Global: ${globalBanners.length}, Area: ${areaBanners.length}, Nearby: ${nearbyBanners.length}, Fallback: ${fallbackBanners.length}`
  );
  ...
  if (remainingSlots > 0) {
    ...
    if (areaBanners.length > 0) {
      console.log(
        `Found ${areaBanners.length} category banners in user's area - adding area banners`
      );
      ...
      console.log(
        "No category banners found in user's area - falling back to nearest banners within 55km"
      );
      ...
      if (nearbyBanners.length > 0) {
        ...
        console.log(
          `Using ${Math.min(
            ...
            remainingSlots
          )} nearest category banners within 55km`
        );
      } else {
        // 3. LAST RESORT: Use fallback banners (banners with no location restrictions)
        console.log(
          "No nearby category banners found within 55km - using unrestricted banners"
        );
  ...
  console.log(
    `Final category banner selection - Total: ${selectedBanners.length}`
  );
...
async function getAllEligibleCategoryBanners(categoryId) {
...
async function getBannerLocationData(banner) {
  try {
    ...
    // Check cache first
    const cacheKey = `category_banner_location_${banner.id}`;
    ...
    if (banner.locationUrl) {
      ...
      if (coordinates) {
        ...
        console.log(
          `✓ Got location data from locationUrl for category banner ${banner.id}`
        );
      }
    }
    ...
    if (!locationData && banner.pincode) {
      ...
      if (pincodeLocation) {
        ...
        console.log(
          `✓ Got location data from pincode ${banner.pincode} for category banner ${banner.id}`
        );
      }
    }
    ...
    if (locationData) {
      ...
      console.log(`Cached location data for category banner ${banner.id}:`, {
      ...
      console.log(
        `⚠️ Could not determine location for category banner ${banner.id}`
      );
    }
    ...
    console.error(
      `Error getting location data for category banner ${banner.id}:`,
      error.message
    );
    return null;
  }
...
export function formatCategoryBanners(banners) {
  return banners.map((banner) => {
    return {
      id: banner.id,
      imageUrl: banner.Image || "/placeholder-banner.jpg",
      title: `${banner.category?.name || "Category"} Banner`,
      subtitle:
...
function calculateBannerLocationScore(userLocation, bannerLocation, banner) {
  ...
  if (
    ...
    console.log(
      `✓ City match for category banner ${banner.id}: ${bannerLocation.city}`
    );
  }
  ...
  if (
    ...
    console.log(
      `✓ Sub-district match for category banner ${banner.id}: ${bannerLocation.subDistrict}`
    );
  }
  ...
  if (userLocation.coordinates && bannerLocation.coordinates) {
    ...
    banner.distance = distance;
    console.log(
      `Distance for category banner ${banner.id}: ${distance.toFixed(2)}km`
    );
