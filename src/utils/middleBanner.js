...
export default async function getFilteredMiddleBanners(
  userPincode,
  maxResults = 10
) {
  const allBanners = await getAllEligibleMiddleBanners();
...
async function getAllEligibleMiddleBanners() {
  const currentDate = new Date();

  return await prisma.middleBanner.findMany({
    where: {
      active: true,
      OR: [
...
async function getBannerLocationData(banner) {
  try {
    ...
    // Check cache first
    const cacheKey = `middle_banner_location_${banner.id}`;
...
export function formatMiddleBanners(banners) {
